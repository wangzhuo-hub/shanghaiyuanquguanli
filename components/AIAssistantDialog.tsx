...
export const AIAssistantDialog: React.FC<AIAssistantDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-[600px] flex flex-col animate-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-slate-200 bg-gradient-to-r from-blue-50 to-indigo-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800">AI 智能助手</h2>
              <p className="text-xs text-slate-500">园区经营数据分析与报告生成</p>
            </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-200">
          <div className="flex gap-2 flex-wrap">
            <button 
              ...
              disabled={isLoading}
              className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg hover:border-blue-300 hover:bg-blue-50 transition-all text-sm font-medium text-slate-700 disabled:opacity-50"
            >
                ...
                {msg.isHTML && msg.id && msg.role === 'assistant' && (
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-1.5 text-xs bg-white border border-slate-300 hover:bg-slate-50 rounded-lg transition-colors"
                    >
                      <ImageIcon size={14} />
                      导出为图片
                    </button>
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-3 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm"
              disabled={isLoading}
            />
            <button
